...
A Go library has no package manifest and is **not** published to a registry: consumers `go get`
the module straight from the tag. The **first release is `v1.0.0` by hand** — release-drafter would
otherwise draft `v0.1.0` on the first run.
